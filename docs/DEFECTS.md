...
| O2 | `generate` ignores parts of the brief: returned 11 questions when asked for exactly 10, and turned the open questions into four-option selects on both surveys tried | live run, twice | A question-count bound in the tool schema; the answer-type ban already exists as `allowed_answer_types` and could be extended to require one |
...
| O9 | A grounding-gate refusal mid-survey reaches the **respondent** as `503 llm_unavailable`: every provider call returned 200 and the gate refused the model's content twice, but the respondent is told the assistant is briefly unavailable and to try again, which re-runs the same turn against the same answer. Their message is lost | live run | The same fix the run summary got: a refusal on the merits is not an outage. The engine cannot simply 409 a respondent mid-conversation, so it probably has to ask a follow-up instead of failing the turn, which is what it does for every other kind of unusable answer |
...
| O11 | An open follow-up records the parent question's typed shape: "Can you share more about why you feel that way?" recorded `{"rating": 2}` | live run, again | Already recorded as O8; seen a second time on 12 Aug 2026, on a different survey and a different model, so it is not a one-off |
...
| O8 | A follow-up asking *why* records the parent question's value: "What made your first week a solid 4?" recorded `{"rating": 4}` | live run | The value is grounded, so no gate objects, but it answers a question nobody asked. A probe that re-asks in prose should not accept the parent's typed shape when the probe text is open |
