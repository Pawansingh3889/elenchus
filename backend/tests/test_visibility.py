...
def test_yes_no_matches_the_word_an_author_would_write(value: dict) -> None:
...
def test_a_number_matches_however_it_was_stored() -> None:
    """4 and 4.0 are the same answer to an author who typed 4."""
...
def test_a_decline_satisfies_neither_operator() -> None:
    ...
    declined = {"unanswerable": "respondent declined"}
...
def test_a_condition_must_want_an_option_the_question_offers() -> None:
    """Otherwise the author only finds the typo by running the survey and noticing the
...
async def _published(session, author):
...
async def test_the_engine_skips_a_question_whose_condition_fails(session, author, respondent):
    template = await _published(session, author)
    ...
    run = await ConductEngine(session, llm=llm).handle_message(run.id, "line operator", respondent)
    ...
    run = await ConductEngine(session, llm=llm).handle_message(run.id, "4", respondent)
...
async def test_the_engine_asks_the_question_when_the_condition_holds(session, author, respondent):
    template = await _published(session, author)
    engine = ConductEngine(session, llm=FakeLLM())
    run = await engine.start_run(template.id, respondent)

    # q0 permits no probing, so the engine advances straight off the record turn and
    # that turn's own text is what the respondent hears.
    llm = FakeLLM(record("Quality Manager", "Got it. What outcomes do you own?"))
    run = await ConductEngine(session, llm=llm).handle_message(run.id, "QM", respondent)
...
async def test_a_completed_conditional_run_reads_n_of_n(session, author, respondent):
    ...
    run = await skip.handle_message(run.id, "line operator", respondent)
