...
This is the first thing in the app that decides who may build surveys. Before it, `role`
was written by `app/seed.py` and by nothing else, so the answer to "who decides who can be
an author" was "whoever can reach the database". It is now an administrator, on a screen,
with the change recorded against their name in `users.created_by`.
...
from app.access import in_audience, is_admin_by_config
...
from app.users.models import AccountChange, User, UserGroupMembership
...
class UserService:
    ...
    async def create_account(self, data: AccountCreate, admin: User) -> User:
        ...
        user = User(
            ...
            created_by=admin.id,
            memberships=[UserGroupMembership(group=g) for g in data.groups],
        )
        self.repo.add(user)
        ...
        logger.info("account created: user=%s role=%s by=%s", user.id, user.role.value, admin.id)
        return user
    ...
    async def replace_account(self, user_id: UUID, data: AccountUpdate, admin: User) -> User:
        ...
        buys on the groups specifically: assigning the collection is what *removes* the
        memberships that are no longer listed, and `cascade="all, delete-orphan"` on the
        relationship turns that into the deletes. A patch that only ever added would make
        moving somebody off a line impossible through this screen.
        """
        ...
        user.memberships = [UserGroupMembership(group=g) for g in data.groups]
        ...
        await self._commit_or_conflict()
        logger.info("account changed: user=%s role=%s by=%s", user.id, user.role.value, admin.id)
        return user
    ...
    async def history(self, user_id: UUID) -> list[AccountChangeRead]:
        ...
        return [
            AccountChangeRead(
                ...
                before=(
                    AccountSnapshot.model_validate(change.change["before"])
                    if change.change["before"] is not None
                    else None
                ),
                after=AccountSnapshot.model_validate(change.change["after"]),
            )
    ...
    async def preview_change(self, user_id: UUID, data: AccountUpdate) -> AccountImpact:
        ...
        an id, and no change of role or groups makes this user a different person.
        ...
        return AccountImpact(
            groups_added=sorted(set(data.groups) - user.groups, key=lambda g: g.value),
            groups_removed=sorted(user.groups - set(data.groups), key=lambda g: g.value),
            role_before=user.role,
            role_after=data.role,
            department_before=user.department,
            department_after=data.department,
            surveys=surveys,
        )
    ...
    def _refuse_self_lockout(self, user: User, admin: User) -> None:
        ...
        nobody can create an account or grant anyone else the department that would let
        them. Fixing it means reaching the database directly, which is the thing this
        screen exists to stop being necessary.

        Only their *own* account, and only administration. An administrator demoting
        themselves to respondent keeps the admin routes, which is enough to undo it, and
