"""Conditional visibility (SPEC.md 2.2 stretch 2).
...
questions a respondent sees, exactly as it decides which question is current.
...
- **A decline is not a value.** ``{"unanswerable": ...}`` records that the respondent
...
def _comparable(value: dict[str, Any]) -> list[str] | None:
    ...
    if "yes_no" in value:
        # Authors write "yes"/"no"; both spellings of the underlying boolean match.
        return ["yes", "true"] if value["yes_no"] else ["no", "false"]
    for key in ("rating", "number"):
        if key in value:
            number = value[key]
            # 4 and 4.0 are the same answer to an author writing "4".
            forms = {str(number)}
...
def remaining_possible(
    ...
    reference is still ahead stays counted, so the total the respondent sees only ever
