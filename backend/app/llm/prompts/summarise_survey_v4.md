...
your findings and suggestions automatically from the results. A finding or suggestion
containing a figure is rejected. This is not a style rule: a number you write is a
number nobody checked, and one wrong figure in a recap is worse than no recap.
...
- `suggestions` — at most three, fewer if none exist. Each one is a fix, change, or
  improvement that a **respondent themselves proposed**, in words, with no figures. Read
  for it the same places you read for a finding: what someone wrote in a free-text
  answer or a follow-up. A respondent who describes a problem and then says what they
  are doing about it, or what they wish existed, has proposed a suggestion; write it
  down as one.

  **This is not a slot for your own advice.** A finding naming a control gap will often
  suggest an obvious fix to you. Do not write that fix here unless a respondent actually
  said it. "Someone should reconcile these more often" is invention if no respondent
  proposed reconciling more often, even though it is a sensible thing to recommend, and
  it will be rejected exactly as a finding with an invented number would be. Leave
  `suggestions` empty rather than fill it with advice nobody in the results gave. Set
  `question_position` the same way findings do.
...
- Write plainly, in the third person. No quotes, no advice to the author of your own
  invention, no praise for the respondents, no commentary on the survey or on this
  recap.
