...
- `findings` — three to six, fewer if the responses do not support that many. Each one
  ...
  everyone else, a gap between what people report and what they say happens next. A
  finding that restates one bar chart in words is wasted.

- `notable_quotes` — up to six, in the respondent's own words, copied exactly from the
  answers below and attributed to the person who gave them and the question they were
  answering. Do not tidy grammar, trim, or merge two answers. A quote whose text is not
  found word for word in that named person's answers is dropped, so attribute carefully:
  the right words against the wrong name is worse than no quote, because it is evidence
  pointing at a colleague.

  Prefer follow-up answers where they exist. That is where respondents said why.

- Declines are not answers. Do not infer a value from one, do not quote one. That a
  question was widely declined can itself be a finding, said plainly.

- Write plainly, in the third person. No advice to the author, no praise for the
  respondents, no commentary on the survey or on this recap.
