...
Stdlib only, and no import from anything else in ``app``. The live harness runs on plain
``python3`` against the HTTP API (see .github/workflows/live-conduct.yml) and reaches this
file by path through ``scripts/conduct_invariants.py``, so anything it shares with the suite
has to survive without the backend's virtualenv. It lives in the app because evaluation
runs check themselves with it too.
